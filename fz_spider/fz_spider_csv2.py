...
for page in range(1, 101):
    ...
    for se in sel:
        ...
        with open("链家二手房.json", mode="a", encoding="utf-8", newline="") as f:
            # 传入一个文件对象f
            # 用csv_writer一个变量去接收一个csv的一个对象f
            csv_writer = csv.writer(f)
            # writerow()是以 一行一行的定入数据
            # 写入数据要有一个容器，写入哪些由自己决定
            csv_writer.writerow([title, addre, address, follow, tags, price_jo, unitprice])
