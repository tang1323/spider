...
import time
import ast
from queue import Queue     # 我们在这里用消息队列做多线程
"""
python中的库还是第三方库
一般在实际中都大量使用queue消息队列来对多线程间通信

这里的Queue己经帮我们实现了线程同步功能，所以我们不用担心线程同步问题
所以我在这里用队列做不用担心
"""
...
# 这是我们左边导航栏模块的总url
# topic_list_urls = []
# 这里我们就能控制我们的并发了，就是只能放10个数据, 这里先不要控制，因为有很多数据要进来
topic_list_queue = Queue()

# 这是首页帖子的全部url
# topic_list = []
# 这里我们就能控制我们的并发了，就是只能放10个数据
...
class ParseAnswerDetailThread(Thread):
    def run(self):
        while 1:
            ...
            # 从answer_queue消息队列一直拿url， 这里我们不用再做try，我们就要它阻塞住
            url = answer_queue.get()
...
class ParseTopicDetailThread(Thread):
    def run(self):
        while 1:
            # 从topic_queue消息队列一直拿url， 这里我们不用再做try，我们就要它阻塞住
            url = topic_queue.get()
...
class ParseAuthorThread(Thread):
    def run(self):
        while 1:
            # 从author_queue消息队列一直拿url， 这里我们不用再做try，我们就要它阻塞住
...
class ParseTopicListThread(Thread):
    def run(self):
        while 1:
            # 从topic_list_queue消息队列一直拿url， 这里我们不用再做try，我们就要它阻塞住
            # 1.先判断一下ctrl+c命令
            ...
            for tr in all_trs:
                ...
                # 把分析下来的帖子详情url传给这个全局队列topic_queue，会有ParseTopicDetailThread拿去解析
                # 这样就是线程的通信了
...
if __name__ == "__main__":
    ...
    # 2.如何去停止线程
        # 1.我们想手动停止 ctrl + c，很优雅的退出
            # 1.接收ctrl + c
            # 2.保存临时变量
            # 3.下一次启动程序的时候，我们是应该如何从文件（mysql）中启动，就是哪里中断，就从哪里开始

    # 2.程序有可能直接崩溃

        # 1.queue是内存中，是否安全的
        # 2.queue这种模式就不合适了，使用mysql来同步， 就是一边添加数据一边取数据
        # 3.redis来做
